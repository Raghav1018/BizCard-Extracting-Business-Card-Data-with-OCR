...
if selected == "Home":
    ...
    with col1:
        st.image(Image.open("image_1.jpeg"), width=500)
        st.markdown("## :green[**Technologies Used :**] Python,easy OCR, Streamlit, SQL, Pandas")
    with col2:
       st.write("## :green[**About :**] Bizcard is a Python application designed to extract information from business cards.")
       st.write("## Bizcard's major objective is to automate the process of removing important information from photos of business cards, including the name, designation, organisation, contact information, and other pertinent data. Bizcard can extract text from the photos by using the OCR (Optical Character Recognition) capabilities offered by EasyOCR.")
...
if selected == "Upload & Extract":
    if st.button(":blue[Already stored data]"):
        ...
        updated_df = pd.DataFrame(cr.fetchall(),
    ...
    if uploaded_card is not None:
        ...
        if st.button("Upload to Database"):
            ...
            for i, row in df.iterrows():
                insert_query= f"""INSERT INTO cards (company_name,card_holder,designation,mobile_number,email,website,area,city,state,pin_code,image) VALUES (%s, %s, %s, %s, %s, %s, %s,%s, %s, %s, %s);"""
                data_tuple = (
                                row['company_name'],
                                row['card_holder'],
                                row['designation'],
                ...
                cr.execute(insert_query, data_tuple)
                st.success("#### Uploaded to database successfully!")
                
                
        if st.button(":blue[View updated data]"):
            cr.execute("select company_name,card_holder,designation,mobile_number,email,website,area,city,state,pin_code from cards")
            updated_df = pd.DataFrame(cr.fetchall(),
                                          columns=["Company_Name", "Card_Holder", "Designation", "Mobile_Number",
                                                   "Email",
                                                   "Website", "Area", "City", "State", "Pin_Code"])
...
if selected == "Modify":
    ...
    if select == "ALTER":
        ...
        try:
            cr.execute("SELECT card_holder FROM cards")
            result = cr.fetchall()
            business_cards = {}
            for row in result:
            ...
            if selected_card == "None":
                ...
                cr.execute(
                "select company_name,card_holder,designation,mobile_number,email,website,area,city,state,pin_code from cards WHERE card_holder=%s",
                (selected_card,))
                result = cr.fetchone() 
                ...
                if st.button(":blue[Commit changes to DB]"):
                     ...
                     cr.execute("""UPDATE cards SET company_name=%s,card_holder=%s,designation=%s,mobile_number=%s,email=%s,website=%s,area=%s,city=%s,state=%s,pin_code=%s
                ...
                if st.button(":blue[View updated data]"):
                    cr.execute(
                        "select company_name,card_holder,designation,mobile_number,email,website,area,city,state,pin_code from cards")
                    updated_df = pd.DataFrame(cr.fetchall(),
                                          columns=["Company_Name", "Card_Holder", "Designation", "Mobile_Number",
                                                   "Email",
                                                   "Website", "Area", "City", "State", "Pin_Code"])
        ...
        try:
            ...
            if selected_card == "None":
                ...
                if st.button("Yes Delete Business Card"):
                    cr.execute(f"DELETE FROM cards WHERE card_holder='{selected_card}'")
